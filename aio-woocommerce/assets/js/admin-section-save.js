...
(function() {
    ...
    function handleSectionSave(event, button) {
        ...
        // Debug: Check if cart_rules is in the payload
        if (payload.cart_rules) {
            console.log('✅ cart_rules found in payload:', typeof payload.cart_rules, payload.cart_rules.substring ? payload.cart_rules.substring(0, 100) + '...' : payload.cart_rules);
        } else {
            console.warn('⚠️ cart_rules NOT in payload!');
            // Fallback: Try to get it directly from the hidden field
            if (cartRulesHiddenField && cartRulesHiddenField.value) {
                console.log('🔍 Found hidden field, adding to payload:', cartRulesHiddenField.value.substring(0, 100) + '...');
    ...
    function collectFormData(form) {
        ...
        // Debug: Log hidden inputs found
        if (legitimateHiddenInputs.length > 0) {
            console.log('🔍 Found', legitimateHiddenInputs.length, 'legitimate hidden inputs:', legitimateHiddenInputs.map(i => i.name));
        }
        ...
        inputs.forEach(input => {
            ...
            if (nestedMatch) {
                ...
                if (input.type === 'checkbox') {
                    data[parentField][childField] = input.checked ? (input.value || 'yes') : 'no';
                } else {
                    if (input.multiple) {
            ...
            // Debug: Log cart_rules field processing
            if (field === 'cart_rules' && input.type === 'hidden') {
                console.log('🔍 Processing cart_rules hidden input:', {
                    name: input.name,
                    value: input.value ? input.value.substring(0, 100) + '...' : 'empty',
                    hasRawValue: input.getAttribute('data-raw-value') !== null
                });
            }
        ...
        allCheckboxes.forEach(checkbox => {
            ...
            if (nestedMatch) {
                // This is a nested object field (like payment_gateway_zarinpal[enabled])
                if (!data[parentField]) {
                    data[parentField] = {};
                }
    ...
    function buildFormData(payload) {
        ...
        if (payload.cart_rules) {
            console.log('📦 cart_rules type:', typeof payload.cart_rules, 'length:', payload.cart_rules.length);
        }
